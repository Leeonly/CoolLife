...
- (void)addSectionButton
{
    ...
    RETableViewItem *buttonItem = [RETableViewItem itemWithTitle:@"查询" accessoryType:UITableViewCellAccessoryNone selectionHandler:^(RETableViewItem *item) {
        ...
        if (weakSelf.numberItem.value) {
            // 查询数据
            NSString *amount =  weakSelf.numberItem.value;
            [weakSelf getCurrencyDataWithFrom:[weakSelf.swapOut.value substringToIndex:3] to:[weakSelf.swapIn.value substringToIndex:3] amount:amount];
...
- (void)getCurrencyDataWithFrom:(NSString *)from to:(NSString *)to amount:(NSString *)amount
{
    
    __typeof (self) __weak weakSelf = self;
    
    
    [LeeSearchHttpTool getCurrencyDataWithFrom:from to:to amount:amount success:^(NSData *data) {
        ...
        if (error.integerValue == 0) {
            // 添加item
            // from货币
            [weakSelf.resultSection addItem:[WBSubtitleItem itemWithTitle:dic[@"fromCurrency"] rightSubtitle:weakSelf.numberItem.value]];
            // 当前汇率
            [weakSelf.resultSection addItem:[WBSubtitleItem itemWithTitle:@"当前兑换汇率" rightSubtitle:dic[@"currency"]]];
            
            // to货币
            //                        double toNumber = [] * [dic[@"now"] doubleValue];
            ...
            // 买入汇率
            //            [weakSelf.resultSection addItem:[WBSubtitleItem itemWithTitle:@"买入" rightSubtitle:dic[@""]]];
            //            // 卖出
...
- (void)dealloc
{
    LeeLog(@"CurrencySearchController dealloc");
}
