...
@interface ProductController() <CitysViewdelegate>
...
- (void)viewDidLoad {
    ...
    // 定位
    [LeeLocationTool sharedLeeLocationTool];
...
- (void)locationCity
{
    LeeLocationCity *locationCity = [LeeLocationTool sharedLeeLocationTool].locationCity;
    if (locationCity && locationCity.city) {
        [self loadWeatherData:[LeeLocationTool sharedLeeLocationTool].locationCity.city];
    }
}
...
{
    [LeeSearchHttpTool getWeatherDataWithCity:city success:^(id json) {
...
//    ProductItem *dreamItem = [ProductItem itemWithTitle:@"周公解梦" icon:@"a6" destVcClass:[DreamAnalysisController class]];
    ...
    [section.items addObjectsFromArray:@[IDCardItem,phoneItem,currencyItem,IPItem]];
